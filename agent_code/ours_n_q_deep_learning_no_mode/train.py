...
from features import breadth_first_search, prepare_field_coins, determine_explosion_timer, \
    count_destroyable_crates_and_enemies, determine_current_square
...
def add_custom_events(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str],
    ...
    count_crates, count_enemies = count_destroyable_crates_and_enemies(x, y, old_game_state, explosion_timer)
    current_square = determine_current_square(x, y, old_game_state, count_crates + count_enemies)
    ...
    if e.WAITED in events and current_square > 0:
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.transitions.append(
        Transition(features_old, torch.Tensor([ACTION_TO_INDEX[last_action]]).to(torch.int64),
                   None, torch.Tensor([reward_from_events(self, events)])))
    # training
    for _ in range(10):
        train_batch(self)
    ...
    if self.round % ROUNDS_PER_SAVE == 0:
        # Store the model
        self.save += 1
        with open("models/" + "{:04d}".format(self.save) + "-network.pt", "wb") as file:
