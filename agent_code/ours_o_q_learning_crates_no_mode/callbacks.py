...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    if current_square > 1 and count_crates > 0:
        worth_move = determine_is_worth_to_move_crates(x, y, game_state, count_crates, explosion_timer)
        features.extend(worth_move)
        has_crates = True
        if worth_move.max() > 0 and priority_marker < 0:
            priority_marker = 2

    if not has_crates:
        crates, min_distance_crates = determine_crate_value(x, y, game_state, explosion_timer)
        features.extend(crates)
        if crates.max() > 0 and priority_marker < 0:
            priority_marker = 2

    has_enemies = False
    if current_square > 1 and count_enemies > 0:
        worth_move = determine_is_worth_to_move_enemies(x, y, game_state, count_enemies, explosion_timer)
        features.extend(worth_move)
        has_enemies = True
        if worth_move.max() > 0 and priority_marker < 0:
            priority_marker = 3
    ...
    if not has_enemies:
        ...
        if enemies.max() > 0 and priority_marker < 0:
            priority_marker = 3
